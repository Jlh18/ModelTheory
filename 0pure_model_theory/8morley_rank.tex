...
\begin{proof}
    ...
    By \linkto{infite_infinite_classes}{quantifier elimination of $T$} and the 
    ...
    $d \in \MM$ or 
    $S_1(\MM)' \subs [\psi]$.
    As $T$ has \linkto{infite_infinite_classes}{quantifier elimination} 
...
\begin{prop}[Implication, subset, and inequality]
    \link{implication_subset_inequality}
    Let $\phi$ and $\psi$ be two $\Si(\MM)$-formulas.
    Suppose $\MM \model{\Si} \forall v, \psi \to \phi$ 
    (equivalently $\psi(\MM) \subs \phi(\MM)$),
    then for any elementary extension $\NN$ of $\MM$
...
\begin{proof}
    ...
    \begin{align*}
        T_{n+1} = T_n &\cup \set{t;0 \st t \in T_n \text{ and }
        \exists \psi \in \form{\Si(\M)}, 
        \MR{}{\Phi_n(t) \AND \psi} = \MR{}{\Phi_n(t) \AND \NOT \psi} =\al}\\
        &\cup \set{t;1 \st t \in T_n \text{ and }
        \exists \psi \in \form{\Si(\M)}, 
        \MR{}{\Phi_n(t) \AND \psi} = \MR{}{\Phi_n(t) \AND \NOT \psi} =\al}
    \end{align*}
    ...
    and $t;1 \mapsto \Phi_{n+1}(t) \AND \NOT \chi_{n+1}$.
    By assumption
    ...
    \[  
        T := \bigcup_{n \in \N} T_n \quad 
        \Phi : T \to \form{\Si(\M)} := \bigcup_{n \in \N} \Phi_n
    \]
    $T$ is a binary tree:
    ...
    \begin{align*}
        ...
        &= \brkt{\bot \OR \Phi_n(\res{b}{n}) \AND \NOT \chi_{n+1}}(\M)\\
    ...
    \begin{itemize}
        \item for $t \in N$ 
            there is no $\Si(\M)$-formula $\chi$
            such that $\MR{}{\Phi(t) \AND \chi}$
